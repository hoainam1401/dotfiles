...
update_track() {
    ...
    if [[ -z $SPOTIFY_JSON ]]; then
        sketchybar --set $NAME icon.color=0xff8aff80 label.drawing=no
        return
    fi
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME label="${TRACK} ▫️ ${ARTIST}" label.drawing=yes icon.color=0xff8aff80

    elif [ $PLAYER_STATE = "Paused" ]; then
        sketchybar --set $NAME icon.color=0xffffff80
    elif [ $PLAYER_STATE = "Stopped" ]; then
        sketchybar --set $NAME icon.color=0xffff9580 label.drawing=no
    else
        sketchybar --set $NAME icon.color=0xff80ffea
    fi
}
